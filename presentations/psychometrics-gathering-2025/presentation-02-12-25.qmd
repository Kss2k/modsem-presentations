...
## Example: LMS approach, using `nlsem` (213.2 seconds)
\scriptsize
```{r}
set.seed(3248927)
library(nlsem)
...
oneIntSorted <- oneInt[c("x1", "x2", "x3", "z1", "z2", "z3", "y1", "y2", "y3")]
...
fit_nlsem <- em(nlsem_model_1, data = oneIntSorted, 
                convergence = 1e-4, max.iter = 1L, start = start)
```
\normalsize
...
## Example: LMS approach, using `modsem` (1.2 seconds)
